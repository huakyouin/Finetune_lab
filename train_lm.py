...
def main(config,resume):
    ...
    if config['base']=='qwen':
        ...
        train_dataset_tokenized = preprocess_data_for_qwen(sources = train_dataset, tokenizer = tokenizer, max_len = config["max_len"] )
    elif config['base']=='llama':
        tokenizer.pad_token = tokenizer.eos_token
        train_dataset_tokenized = preprocess_data_for_llama(sources = train_dataset, tokenizer = tokenizer, max_len = 512 )
    else:
